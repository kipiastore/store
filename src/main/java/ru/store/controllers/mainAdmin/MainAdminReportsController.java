 ...
 */
@Controller
public class MainAdminReportsController {
    ...
    @RequestMapping(value = "/mainAdmin/reports", method = RequestMethod.GET)
    public ModelAndView reports() {
        ModelAndView modelAndView = new ModelAndView();
        MainAdminPositionsController.Model model = new MainAdminPositionsController.Model();
        loadPage(model, modelAndView);
        return modelAndView;
    }

    @RequestMapping(value = "/mainAdmin/addreport", method = RequestMethod.POST)
    ...
    @RequestMapping(value = "/mainAdmin/addreport", method = RequestMethod.GET)
    public String redirect1() {
        return "redirect:/mainAdmin/reports";
    }
    @RequestMapping(value = "/mainAdmin/reportssearchcompany", method = RequestMethod.GET)
    ...
    private void loadPage(MainAdminPositionsController.Model model, ModelAndView modelAndView) {
        model.selectedPageNum = 9;
        loadCompanies(model);
        modelAndView.addObject("model", model);
        modelAndView.addObject("prefix", "");
        modelAndView.setViewName("mainAdmin/reports");
    }

    private void loadCompanies(MainAdminPositionsController.Model model) {
        model.message = "Последние редактированные фирмы:";
        model.companyList = new ArrayList<>();
        for (Company company : companyService.getCompaniesByLastUpdate()) {
            model.companyList.add(convert(company));
        }
        List<MainAdminPositionsController.Model.PartitionItem> partitionItems = new ArrayList<>();
        MainAdminPositionsController.Model.PartitionItem partitionItem;
        MainAdminPositionsController.Model.PartitionItem mainPartitionItem;
        Map<Integer, MainAdminPositionsController.Model.PartitionItem> partitionIdToPartitionItem = new HashMap<>();
        Map<MainAdminPositionsController.Model.PartitionItem, List<MainAdminPositionsController.Model.PartitionItem>> subPartitionsGroupedByPartition = new HashMap<>();
        for (Partition partition : partitionService.getPartitions()) {
            partitionItem = new MainAdminPositionsController.Model.PartitionItem();
        ...
        for (SubPartition subPartition : subPartitionService.getSubPartitions()) {
            if (subPartitionsGroupedByPartition.get(new MainAdminPositionsController.Model.PartitionItem(subPartition.getPartitionId())) != null) {
                partitionItem = new MainAdminPositionsController.Model.PartitionItem();
                partitionItem.id = subPartition.getId();
                partitionItem.name = getNormalName(subPartition.getName());
                subPartitionsGroupedByPartition.get(new MainAdminPositionsController.Model.PartitionItem(subPartition.getPartitionId())).add(partitionItem);
            } else {
                mainPartitionItem = new MainAdminPositionsController.Model.PartitionItem();
    ...
    private MainAdminPositionsController.Model.CompaniesItem convert(Company company) {
        MainAdminPositionsController.Model.CompaniesItem companyItem = new MainAdminPositionsController.Model.CompaniesItem();
