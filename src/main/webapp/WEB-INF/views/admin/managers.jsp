    ...
    <div class="menuBody">
        <div class="generalContent">
            ...
            <form:form action="addmanager" modelAttribute="user" method="post" id="createForm">
                ...
                <input title="Логин менеджера может содержать латинские буквы, цифры, знаки дефиса, подчеркивания. От 4 до 20 символов."
                ...
                <input title="Пожалуйста, введите тот же пароль, как указано выше." type="password" id="password2" required
            ...
            <form:form action="updatemanager" modelAttribute="user" method="post" id="updateForm">
                <label>Ф.И.О.</label>
                ...
                <input id="username" type="text" disabled pattern="(^[\w+]{4,20}$)" name="username"
                       title="Логин менеджера может содержать латинские буквы, цифры, знаки дефиса, подчеркивания. От 4 до 20 символов.">
                <label>Новый пароль</label>  <input type="checkbox"  name="showPassword" id="showPasswordUpdate" /> <label>Показать пароль</label>
                <input title="Пароль должен содержать не менее 6 символов, включая верхний/нижний регистр и цифры."
                       type="password"  pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{6,}" name="password"id="passwordUpdate"
                       onchange="this.setCustomValidity(this.validity.patternMismatch ? this.title : '');
                            if(this.checkValidity()) form.password2.pattern = this.value;">
                <label>Подтвердите новый пароль</label>
                <input  title="Пожалуйста, введите тот же пароль, как указано выше." type="password"
                        pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{6,}" name="password2"id="passwordUpdate2"
                        onchange="this.setCustomValidity(this.validity.patternMismatch ? this.title : '');">
                <label>Доступ</label>
...
<div class="menu">
    <div class="menuTitle">
        <span class="menuTitleText">Менеджеры</span>
    </div>
    <div class="menuBody">
        <form:form action="deletemanager" method="post" id="deleteForm">
